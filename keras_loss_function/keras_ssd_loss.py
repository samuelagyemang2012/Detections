...
class SSDLoss:
    ...
    def compute_loss(self, y_true, y_pred):
        ...
        '''
        self.neg_pos_ratio = self.neg_pos_ratio  # tf.constant(self.neg_pos_ratio)
        self.n_neg_min = self.n_neg_min  # tf.constant(self.n_neg_min)
        self.alpha = self.alpha  # tf.constant(self.alpha)

        batch_size = tf.shape(y_pred)[0]  # Output dtype: tf.int32
        n_boxes = tf.shape(y_pred)[1]
        # Output dtype: tf.int32, note that `n_boxes` in this context denotes the total number of boxes per
        # image, not the number of boxes per cell.

        # 1: Compute the losses for class and box predictions for every box.

        classification_loss = tf.cast(self.log_loss(y_true[:, :, :-12], y_pred[:, :, :-12]), tf.float32)
        # Output shape: (batch_size, n_boxes)
        localization_loss = tf.cast(self.smooth_L1_loss(y_true[:, :, -12:-8], y_pred[:, :, -12:-8]), tf.float32)
        # Output shape: (batch_size, n_boxes)
        ...
        positives = tf.cast(tf.reduce_max(y_true[:, :, 1:-12], axis=-1), tf.float32)
